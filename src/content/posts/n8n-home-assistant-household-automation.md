...
Two systems that came out of this combination, one mundane and one nerdy.

## System 1: the household agent

The problem is universal: shopping lists, dinner ideas, chores and "we should really fix the gutter" all live in different apps, or in nobody's head. My requirement was a system my girlfriend would actually use, which rules out anything that needs opening the right app in the right mode.
...
**Storage is Nextcloud, not a custom database.** Shopping list, chores and an inbox are Nextcloud Tasks lists (which is CalDAV under the hood). Bigger projects are Deck boards. Dinner planning is a "Meals" calendar, and recipes live in Nextcloud Cookbook. Using open standards here pays off twice: any CalDAV app can see the lists, and Home Assistant has native integrations for both to-dos and calendars, so everything shows up in HA for free.

**n8n is the middleman.** A workflow polls Telegram, feeds new messages to an LLM that classifies and extracts items, writes to Nextcloud via CalDAV and the Deck/Cookbook APIs, and replies in the group. When the agent needs a decision ("I found three curry recipes, which one?") it renders the options as inline keyboard buttons in Telegram, so answering is one tap.

**The display is a Home Assistant dashboard.** A "household planner" dashboard shows the shopping list, chores and the dinner week at a glance, on the wall tablet and in the HA app. We learned a lesson here: I initially had the bot maintain a pinned, constantly-updated list message in the chat, and it turned out nobody wanted that. Chat is for *doing* things, the dashboard is for *seeing* things. Separating those roles is what made the system stick.

**The clever bit: purchase memory.** Every item that gets checked off the shopping list is logged to an n8n data table. When meal planning adds ingredients for a recipe, the workflow skips things bought in the last week and pantry staples, and mentions the skip ("you bought parmesan on Tuesday"). This tiny feature removes the main annoyance of automated shopping lists, which is that they don't know what your kitchen already contains.
...
- **n8n data tables are your state.** Model weights, purchase history, dedup flags, bot offsets. No extra database needed for any of this.
...
- **Let an AI agent do the n8n clicking.** Building nodes by hand in the n8n editor is fine; describing the workflow to Claude Code with an n8n MCP server attached and reviewing what it deploys is faster, and it writes the validation logic you'd skip.
